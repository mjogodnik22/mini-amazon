...
class NewFilterForm(FlaskForm):
    category = SelectField(_l('Filter by Category'))
    key = SelectField(_l('Sort by Key'), choices = [('name','Name'),('pid','ID'),('price','Price'),('pid','Clear Sort')])
    page_val = IntegerField(_l('Jump to Page'))
    search_val = StringField(_l('Search:'))
...
def productPage(page_num = 1, sort_by = 'pid',filter_by = 'None'):
    ...
    filter_categories.append((None, 'Select', 'Clear Filter'))
    ...
    if filterform.validate_on_submit():
        print(filterform.search_val.data)
        if next_page > max_pages:
            next_page = 0
        if filterform.category.data != "":
            filter_by = filterform.category.data
        if filterform.key.data != "":
            sort_by = filterform.key.data
        if filterform.page_val.data != "":
            page_num = filterform.page_val.data
        if filterform.search_val.data != "":
            return redirect(url_for('productPage.productSearchPage', page_num = page_num, query = filterform.search_val.data))
    ...
    return render_template('productpage.html',form1=filterform,
